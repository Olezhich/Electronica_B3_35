...
export function KeyHandler({prev, key, SelfState}){
    ...
    X_res.mantissa = X_res.mStr === '' ? 0 : Number(X_res.mStr);
    X_res.dStr = (X_res.dStr === '0' || X_res.dStr === '-0' ? '' : X_res.dStr);
    X_res.degree = X_res.dStr === '' ? 0 : Number(X_res.dStr);
    X_res.mOverflow = IsOverflow(X_res.mantissa);
